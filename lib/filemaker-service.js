...
export class FileMakerService {
    ...
    jwtSecret;     // Shared secret for JWT validation
    ...
    constructor() {
        ...
        this.jwtSecret = process.env.FILEMAKER_JWT_SECRET || '';
    }
    ...
    async authenticateFileMakerUser(username, password) {

        //Step 1: Establish FileMaker session via Data API
        const authtoken = await this.createSession("auth");
        ...
        try {
            ...
            if (jwtResponse.ok) {
                ...
                // console.log(unvalidatedJWT);

                await this.revokeSession(authtoken);
                const validatedJWT = this.validateJWT(unvalidatedJWT)// now pass the JWT to the next step.
                return validatedJWT;
    ...
    // ============================================================================
    // SESSION JWT VALIDATION - Checks if returned response from filemaker is a valid JWT
    // ============================================================================
    ...
    async validateJWT(jwt) {
        ...
        try {
            ...
            const payload = verify(jwt, this.jwtSecret);
